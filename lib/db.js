...
const trips = {
    create: async (name, destination = '', startDate = '', endDate = '', timezone = 'UTC', location = null) => {
        const id = generateId('trip');
        const query = `
            INSERT INTO trips (id, name, destination, start_date, end_date, timezone, location)
            VALUES ($1, $2, $3, $4, $5, $6, $7)
            RETURNING *
        `;
        const { rows } = await getPool().query(query, [id, name, destination, startDate, endDate, timezone, location]);
